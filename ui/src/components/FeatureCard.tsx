...
export function FeatureCard({ feature, onClick, isInProgress, allFeatures = [], activeAgent }: FeatureCardProps) {
  ...
  return (
    <Card
      onClick={onClick}
      className={`
        cursor-pointer transition-all duration-200 glow-border py-3
        ${isInProgress ? 'animate-pulse' : ''}
        ${feature.passes ? 'border-primary/40' : ''}
        ${feature.needs_human_input ? 'border-amber-500/40' : ''}
        ${isBlocked && !feature.passes && !feature.needs_human_input ? 'border-destructive/40 opacity-80' : ''}
        ${hasActiveAgent ? 'ring-1 ring-primary/50 ring-offset-1 ring-offset-background' : ''}
      `}
    >
